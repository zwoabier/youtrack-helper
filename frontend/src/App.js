...
import { THEME_TAILWIND, PRIORITY_TAILWIND } from '@/utils/theme';
...
function SearchInterface({ tickets }) {
    ...
    return (_jsxs(CommandMenu, { children: [_jsx(CommandPrimitive.Input, { ref: inputRef, value: search, onValueChange: setSearch, placeholder: "Search YouTrack tickets...", className: `h-12 w-full border-none ${THEME_TAILWIND.bgSurface} px-4 py-3 text-lg outline-none ${THEME_TAILWIND.textPrimary}`, autoFocus: true }), _jsxs(CommandPrimitive.List, { ref: resultsContainerRef, className: "max-h-[300px] overflow-y-auto", children: [filteredTickets.length === 0 ? (_jsx(CommandPrimitive.Empty, { className: THEME_TAILWIND.textSecondary, children: "No results found." })) : (_jsxs("div", { className: `text-xs ${THEME_TAILWIND.textSecondary} px-4 py-2`, children: ["Showing ", filteredTickets.length, " result", filteredTickets.length !== 1 ? "s" : ""] })), filteredTickets.map((ticket, index) => (_jsx("div", { ref: index === selectedIndex ? selectedItemRef : null, onClick: () => handleTicketSelect(ticket), className: cn(`flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors border-l-3 border-l-transparent`, index === selectedIndex
                            ? `bg-[hsl(var(--color-bg-hover))] border-l-[hsl(var(--color-accent-bright))]`
                            : `hover:bg-[hsl(var(--color-bg-hover))]`), children: _jsxs("div", { className: `flex flex-col flex-grow group ${THEME_TAILWIND.textPrimary}`, children: [_jsxs("div", { className: "flex justify-between items-center gap-3", children: [_jsx("span", { className: `font-bold ${THEME_TAILWIND.accent} w-24 flex-shrink-0`, children: ticket.id }), _jsx("span", { className: `flex-grow truncate ${THEME_TAILWIND.textPrimary}`, children: ticket.summary }), _jsx("span", { className: `ml-4 text-right flex-shrink-0 ${PRIORITY_TAILWIND[ticket.priority] || PRIORITY_TAILWIND['']}`, children: ticket.priority })] }), _jsxs("div", { className: `flex justify-between items-center text-sm ${THEME_TAILWIND.textSecondary}`, children: [_jsx("span", { children: ticket.type }), _jsx("div", { className: "flex space-x-1", children: ticket.sprints && ticket.sprints.length > 0
                                                ? ticket.sprints.map((sprint) => (_jsx("span", { className: `px-2 py-0.5 border border-[hsl(var(--color-border))] rounded-full text-xs ${THEME_TAILWIND.textSecondary}`, children: sprint }, sprint)))
                                                : null })] })] }) }, ticket.id)))] })] }));
}
...
const CommandMenu = React.forwardRef(({ className, ...props }, ref) => (_jsx(CommandPrimitive, { ref: ref, className: cn(`flex h-full w-full flex-col overflow-hidden rounded-md ${THEME_TAILWIND.bgElevated}`, className), ...props })));
